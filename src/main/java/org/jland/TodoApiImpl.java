...
public class TodoApiImpl implements TodoApi {
  ...
  public Todo getTodo(Integer todoId) {
    return new Todo().completed(false).id(todoId).description("This is a test todo").name("Test Todo");
  }

  @Override
  public List<Todo> getTodos(Boolean completed) {    
    return Arrays.asList(new Todo().completed(false).id(12345).description("This is a test todo").name("Test Todo"));
  }
